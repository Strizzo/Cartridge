...
if [[ "$DUMP" == "1" ]]; then
    for f in "${DEST}/crash.log" "${DEST}/setup.log" "/tmp/cartridge_wifi.log"; do
        echo
        echo "===== ${f} ====="
    ...
    dev_ssh "journalctl -u cartridge-boot -n 40 --no-pager 2>/dev/null || echo '(no journal for cartridge-boot)'"
    exit 0
fi

info "Following journalctl -u cartridge-boot on ${HOST} (Ctrl-C to stop)"
dev_ssh "journalctl -u cartridge-boot -f -n 30 --no-pager"
